#!/usr/bin/env bash
#
# create_iam_user_for_bedrock_ec2.sh
#
# 功能：
...
#
# 用法 (示例):
#   1)  bash <(curl -sSL https://raw.githubusercontent.com/yordyi/AWSBedrock/main/create_iam_user_for_bedrock_ec2.sh) \
#         --user-prefix=myproj-user --region=us-west-2
#
#   2)  先下载后执行:
#       curl -sSL https://raw.githubusercontent.com/yordyi/AWSBedrock/main/create_iam_user_for_bedrock_ec2.sh \
#         -o create_iam_user_for_bedrock_ec2.sh
#       chmod +x create_iam_user_for_bedrock_ec2.sh
#       ./create_iam_user_for_bedrock_ec2.sh --user-prefix=myproj-user --region=us-west-2
#
# 参数:
#   --help               显示帮助
...
# 7. 依次附加策略(仅保留前两个权限)
#######################################
echo "==> 为用户 $IAM_USER_NAME 附加策略 AmazonBedrockFullAccess, AmazonEC2FullAccess"
...
echo "附加策略:           AmazonBedrockFullAccess, AmazonEC2FullAccess"
